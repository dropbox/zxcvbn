(function() {
  ...
  spatial_match = function(password) {
    ...
    if (best.length) {
      ...
      return best.concat(unidirectional_best);
    } else {
      return [];
    }
  };
  ...
  h4x0r_match = function(password) {
    var best, best_coverage, best_sub, candidate, candidates, coverage, i, j, match, matcher, sub, _i, _j, _k, _l, _len, _len2, _len3, _len4, _ref, _ref2;
    best = [];
    best_sub = null;
    best_coverage = 0;
    ...
    for (_l = 0, _len4 = best.length; _l < _len4; _l++) {
      ...
      match.token = password.slice(i, j + 1 || 9e9);
      match.h4x0rd = true;
      match.sub = sub;
    }
    return best;
